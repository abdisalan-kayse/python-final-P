    ...
    def collisions(self):
        ...
        if pygame.sprite.spritecollide(self, self.paddles, False):
            ...
            if abs(self.rect.right - collision_paddle.left)< 10 and self.speed_x > 0:
                self.speed_x *= -1
            if abs(self.rect.left - collision_paddle.right)
            < 10 and self.speed_x < 0:
                self.speed_x *= -1
            if abs(self.rect.top - collision_paddle.bottom)
            < 10 and self.speed_y < 0:
                self.rect.top = collision_paddle.bottom
                self.speed_y *= -1
            if abs(self.rect.bottom - collision_paddle.top)
            < 10 and self.speed_y > 0:
...
class GameManager:
    ...
    def draw_score(self):
        ...
        opponent_score_rect = opponent_score.get_rec
        t(midright=(screen_width / 2 - 40, screen_height/2))
...
tennis = Ball
('tennis.png', screen_width/2, screen_height/2, 4, 4, paddle_group)
tennis_sprite = pygame.sprite.GroupSingle()
tennis_sprite.add(tennis)

game_manager = GameManager(tennis_sprite, paddle_group)
